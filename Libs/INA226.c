/**
  ...
  * @details
			...
			protocol.
//			The accuracy of current measurement is 0.1%, and voltage measurement is 0.8%.
	******************************************************************************
	*/
	
#include "INA226.h"
	
int32_t INA226_getBusV(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getBusVReg(I2CHandler, DevAddress));
}

int32_t INA226_getCurrent(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getCurrentReg(I2CHandler, DevAddress)*INA226_CURRENTLSB_INV);
}

int32_t INA226_getPower(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
