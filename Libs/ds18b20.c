...
* File Name          : ds18b20.c
* Author             : nimaltd					
* Edited by          : polyakovyg
* Date First Issued  : 27/12/2023
* Description        : This file provides use temperature sensor ds18b20.
* Links							 : https://github.com/nimaltd/ds18b20/blob/master/ds18b20.c
*										 : 
********************************************************************************
* History:
* 27/12/2023: V0.1
...
//#define	DS18B20_UPDATE_INTERVAL_MS					1000		//  (((	if==0  >> Ds18b20_ManualConvert()  )))    ((( if>0  >> Auto refresh )))
//#define	DS18B20_GPIO												ONE_WIRE_GPIO_Port
//#define	DS18B20_PIN													ONE_WIRE_Pin
//#define	DS18B20_TIMER												htim1		//Init timer 1us per tick. Example 72 MHz, Prescaler=(72-1), Counter period=Max.				
//---end DS18B20--------------------------------------------------------//
//---main()--------------------------------------------------------//
////DS18B20
//Ds18b20_Init(osPriorityBelowNormal);
//---end main--------------------------------------------------------//
//---Task--------------------------------------------------------//
//DS18B20
...
//	{
//		xQueueReceive(QueueDS18B20Handle, &TemperatureData, portMAX_DELAY);
...
{
	QueueDS18B20Handle = osMessageQueueNew (DS18B20_MAX_SENSORS, sizeof(Ds18b20Sensor_t), &QueueDS18B20_attributes);
...
#else
bool	Ds18b20_Init(uint32_t Priority)
{
	OneWire_t OneWire;
...
void Task_Ds18b20(void * argument)
{
	...
	do
	{
		...
		while (OneWireDevices)
		{
			...
			TempSensorCount++;
			OneWire_GetFullROM(&OneWire, ds18b20[TempSensorCount-1].Address);
			//!OneWire_GetFullROM(&OneWire, argument->attr_bits->ds18b20[TempSensorCount-1].Address);
			OneWireDevices = OneWire_Next(&OneWire);
		}
		if(TempSensorCount>0)
			break;
	...
	if(Ds18b20TryToFind==0)
		...
		if (xQueueSend(QueueDS18B20Handle, &(ds18b20[0]), portMAX_DELAY) != pdPASS)
			{
				//!assert_param( 1u );
			}
		assert_param( 1u );
    ...
    while (!DS18B20_AllDone(&OneWire))
		...
		if(Ds18b20Timeout>0)
		{
			for (uint8_t i = 0; i < TempSensorCount; i++)
			{
				...
				{
					if (xQueueSend(QueueDS18B20Handle, &(ds18b20[i]), portMAX_DELAY) != pdPASS)
					{
						//!assert_param( 1u );
					}
				}
